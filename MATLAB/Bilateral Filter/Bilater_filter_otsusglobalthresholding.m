clc;
clear all;
close all;

Image = double(imread('sample6.png'))/255;
%figure, imshow(Image);

%If coloured image, convert it to gray
if size(Image,3)== 3
    GrayImage = rgb2gray(Image);
    %figure, imshow(GrayImage);
    Image = GrayImage;
end

%Median Filter
%MedianFilteredImage = medfilt2(Image);
%figure,imshow(MedianFilteredImage);

BilateralFilteredImage = BfilterGray(Image, 3, 3, 10);
%figure, imshow(BilateralFilteredImage)

%Otsu's global thresholding algorithm
level = graythresh(BilateralFilteredImage); %Finds the optimal thresholding point
BW = im2bw(BilateralFilteredImage,level); %Thresholding the image based on that value
figure, imshow(BW);